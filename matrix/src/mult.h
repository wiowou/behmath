...
public:
  ...
  void operator()( Vec1<T> &W, Vec2<T> &X, Vec3<T> &Y, Vec4<T> &Z, unsigned long long start = 0  )
  {
  ...
  //! Z = X * Y * W
  template< template <typename> class Vec1, template <typename> class Vec2, template <typename> class Vec3, template <typename> class Vec4 >
  void operator()( unsigned long long end, Vec1<T> &W, Vec2<T> &X, Vec3<T> &Y, Vec4<T> &Z )
  {
    operator()( end, W, X, Z );
    operator()( end, Z, Y );
  }  
  ...
  //! X = X * Y
  void operator()( SparseVect<T> &X, SparseVect<T> &Y, unsigned long long start = 0  ) //tested
  {
    ...
    while ( i < X.NNZ() && j < Y.NNZ() )
    {
      ...
      else if ( X.m_pos[i] < Y.m_pos[j] )
      {
        ...
        --X.m_nnz;
        for ( unsigned long long k = i; k < X.NNZ(); ++k )
        {
  ...
  //! X = X * Y
  void operator()( unsigned long long end, SparseVect<T> &X, SparseVect<T> &Y ) //tested
  {
    unsigned long long i = 0;
    unsigned long long j = 0;
  ...
  //! X = X * Y
  void operator()( SparseVect<T> &X, Vect<T> &Y, unsigned long long start = 0  ) //tested
  {
    unsigned long long j = 0;
    if ( start != 0 )
    {
  ...
  //! X = X * Y
  void operator()( unsigned long long end, SparseVect<T> &X, Vect<T> &Y ) //tested
  {
    unsigned long long j = 0;
  ...
  void operator()( Vect<T> &X, SparseVect<T> &Y, unsigned long long start = 0 ) //tested
  {
    unsigned long long j = 0;
    if ( start != 0 )
    {
  ...
  void operator()( unsigned long long end, Vect<T> &X, SparseVect<T> &Y ) //tested
  {
    unsigned long long j = 0;
  ...
  void operator()( Vect<T> &X, Vect<T> &Y, unsigned long long start = 0  )
  {
    for ( unsigned long long i = start; i < X.Size(); ++i )
    {
      X[i] *= Y[i];
  ...
  void operator()( unsigned long long end, Vect<T> &X, Vect<T> &Y  )
  {
    //end = Z.Size() < end ? Z.Size() : end;
    for ( unsigned long long i = 0; i < end; ++i )
    {
  ...
  void operator()( SparseVect<T> &X, SparseVect<T> &Y, Vect<T> &Z, unsigned long long start = 0  ) //tested
  {
    unsigned long long i = 0;
    unsigned long long j = 0;
    if ( start != 0 )
  ...
  void operator()( unsigned long long end, SparseVect<T> &X, SparseVect<T> &Y, Vect<T> &Z ) //tested
  {
    unsigned long long i = 0;
    unsigned long long j = 0;
    unsigned long long iend = X.NNZ();
  ...
  void operator()( Vect<T> &X, SparseVect<T> &Y, SparseVect<T> &Z, unsigned long long start = 0  ) //tested
  {
    unsigned long long j = 0;
    if ( start != 0 )
    {
  ...
  void operator()( SparseVect<T> &X, Vect<T> &Y, SparseVect<T> &Z, unsigned long long start = 0  ) //tested
  {
    operator()( Y, X, Z, start );
  }

  void operator()( unsigned long long end, Vect<T> &X, SparseVect<T> &Y, SparseVect<T> &Z ) //tested
  {
    unsigned long long j = 0;
    unsigned long long jend = Y.NNZ();
    if ( Y.m_pos[jend - 1] >= end )
  ...
  void operator()( unsigned long long end, SparseVect<T> &X, Vect<T> &Y, SparseVect<T> &Z  ) //tested
  {
    operator()( end, Y, X, Z );
  }
